...
const ShopContextProvider = (props) => {
  ...
  useEffect(() => {
    fetch("https://ecommerce-web-puce.vercel.app/allproducts")
      .then((response) => response.json())
      .then((data) => setAll_Product(data));

    const authToken = localStorage.getItem("auth-token");
    if (authToken) {
      fetch("https://ecommerce-web-puce.vercel.app/getcart", {
        method: "POST",
        headers: {
  ...
  const fetchAllProducts = async () => {
    try {
      const response = await fetch("https://ecommerce-web-puce.vercel.app/allproducts");
      if (!response.ok) {
  ...
  const addToCart = (itemId, size, price, quantity) => {
    ...
    const authToken = localStorage.getItem("auth-token");
    if (authToken) {
      fetch("https://ecommerce-web-puce.vercel.app/addtocart", {
        method: "POST",
        headers: {
          Accept: "application/json",
          Authorization: `${authToken}`,
  ...
  const removeFromCart = (itemId) => {
    ...
    const authToken = localStorage.getItem("auth-token");
    if (authToken) {
      fetch("https://ecommerce-web-puce.vercel.app/removefromcart", {
        method: "POST",
        headers: {
          Accept: "application/json",
          Authorization: `${authToken}`,
